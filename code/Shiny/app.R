      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            solidHeader = TRUE,
            width = 12,
            p("This project aims to analyze and predict trends in the ISU Dining meal plan. By examining meal plan purchases from Fall 2021 to Spring 2025, residence hall occupancy, and churn rates over semesters and years, we will identify the most popular meal plans, analyze student retention, and forecast meal plan sales and ISU Dining income for future terms.
            ...
            p("Several assumptions were made to simplify analysis and draw clear conclusions. All observations in the data set from ISU Dining are assumed
              to be undergraduate students so the Iowa Board of Regents data could be used for total undergraduate count. All students are also treated independently.
...
server <- function(input, output, session) {
  ...
  output$retention_forecast_plot <- renderPlotly({
    ...
    p <- ggplot(sim_df, aes(x = Time, y = fct_relevel(State, "NA", "25 Meal Blocks", "50 Meal Blocks", "100 Meal Blocks", "Campanile", "Gold", "Cardinal"), group = 1,

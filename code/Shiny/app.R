...
# Load datasets
clean_data <- read.csv("CleanDiningData.csv")
current_data <- read.csv("CurrentDiningData.csv")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Predictive Models", icon = icon("cogs"),
               ...
               menuSubItem("Personalized Model", tabName = "personalized_model", icon = icon("user"))
      ),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      # ===== PERSONALIZED MODEL TAB =====
      tabItem(
        tabName = "personalized_model",
        fluidRow(
          box(
            title = "Personalized Meal Plan Recommendation Model",
            status = "primary",
            solidHeader = TRUE,
            width = 12,
            p("This is a personalizable model option where the user is able to choose the model and alter hyperparameters(if available for the given model)."),
            p("The goal is to have a more customizable feel to the dashboard where the user is able to interact with the data in a more 1 on 1 fashon.")
          )
        ),
        fluidRow(
          box(
            ...
            solidHeader = TRUE,
            width = 3,
            selectInput("personalized_term", "Select Term:",
                        choices = c("Fall 2025", "Spring 2026"),
                        selected = "Fall 2025"),
            selectInput("personalized_housing", "Select Housing Location:",
                        choices = NULL, # Will be populated in server
                        multiple = TRUE,
                        selected = NULL),
            sliderInput("confidence_level", "Recommendation Confidence (%)",
                        min = 50, max = 95, value = 75, step = 5),
            actionButton("run_personalized", "Generate Recommendations",
          ...
          box(
            ...
            solidHeader = TRUE,
            width = 9,
            plotlyOutput("personalized_recommendation_plot", height = "350px"),
            verbatimTextOutput("personalized_summary")
          )
        ),
        fluidRow(
          tabBox(
            ...
            id = "personalizedTabs",
            width = 12,
            tabPanel("Recommendation Distribution",
                     plotlyOutput("personalized_dist_plot", height = "350px")),
            tabPanel("Top Recommendations by Location",
                     plotlyOutput("personalized_location_plot", height = "350px")),
            tabPanel("Confidence Analysis",
                     plotlyOutput("personalized_confidence_plot", height = "350px"))
          )
        )
      ),
